...
| ADR-010 | Engine B introspects via RPC, not a direct DB password | accepted | 2026-09-11 | Amirreza |
...
ADR-002 (auth), ADR-006 (anonymisation), ADR-009 (screen mapping → M2) and ADR-010 (Engine B via RPC)
are now recorded as accepted. Remaining candidates to write soonest: **ADR-007** (personas +
stress-test as the M2 build gate) and **ADR-008** (design system single source of truth) — both
decisions are already implemented, only the records are missing. (RLS — the former ADR-003 — is
resolved as ADR-003.)
